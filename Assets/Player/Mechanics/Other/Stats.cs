...
public struct WeaponStats
{
    ...
    public float precision;     // how much bonus intel enemies drop
    ...
    {
        return $"Damage: {damage:F2}%, Size: {size:F2}%, Speed: {speed:F2}%, " +
               $"AttackSpeed: {attackSpeed:F2}%, Duration: {duration:F2}%, " +
               $"Count: {count}x, Precision: {precision:F2}%";
    }
}
...
public struct CharacterStats
{
    ...
    public override string ToString()
    {
        return $"Health: ({flatHealth}, {percentHealth:F2}%), Health: ({flatShield}, x{percentShield:F2}%)" +
